package br.edu.unifesspa.malves.photovoltaics;

/**
 * 
 * @author Marcela Alves
 * @since 2016-07-13
 *
 */
public class Meter {

	/**
	 * Meter Installation Cost (Brazilian Real - BRL)
	 */
	public static final double custoInstalacao = 300;
	
	/**
	 * Purchase cost of kWh (Brazilian Real - BRL)
	 */
	public static final double custoKwhCompra = 0.6770348148;
	
	/**
	 * Sales cost of kWh (Brazilian Real - BRL)
	 */
	public static final double custoKwhVenda = 0.47;
	
	/**
	 * Consumo Mínimo Diario por Medidor (KwH)
	 */
	public static final double consumoMinimo = 3.287671232;
}